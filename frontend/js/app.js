...
function displayResults(data) {
    ...
    container.innerHTML = `
        <div class="results-content">
            <div class="risk-score risk-${riskClass}">${risk.total_score}/100</div>
            <div style="text-align: center; margin-bottom: 1.5rem;">
                <span class="decision-badge ${decisionClass}">${decision}</span>
                <p style="color: #718096; font-size: 0.875rem; margin-top: 0.5rem;">Confidence: ${risk.confidence}</p>
            </div>
            ...
            <div class="result-item">
                ...
                <span class="result-value ${data.tampering.tampering_score > 30 ? 'risk-high' : 'risk-low'}">${data.tampering.tampering_score}%</span>
            </div>
            <div class="result-item">
            ...
            ${data.validation.errors.length > 0 ? `
                <div style="margin-top: 0.75rem; padding: 0.75rem; background: #fed7d7; border-radius: 8px;">
                    <p style="color: #9b2c2c; font-size: 0.875rem; font-weight: 500;">⚠️ Issues: ${data.validation.errors.join(', ')}</p>
                </div>
            ` : ''}

            <div style="margin-top: 1rem; padding: 0.75rem; background: #edf2f7; border-radius: 8px;">
                <p style="font-size: 0.75rem; color: #718096;">Screening ID: ${data.screening_id}</p>
                <p style="font-size: 0.75rem; color: #718096;">Timestamp: ${new Date(data.timestamp).toLocaleString()}</p>
            </div>
        </div>
    `;
}
