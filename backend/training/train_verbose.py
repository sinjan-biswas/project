...
# Lighter augmentation — documents are already realistic; heavy aug destroys MRZ & small text.
...
def main():
    ...
    ap.add_argument("--lr-head", type=float, default=1e-3,
                    help="LR for frozen-backbone warmup phase")
    ap.add_argument("--lr-full", type=float, default=3e-4,
                    help="Peak LR for full fine-tune phase")
    ...
    print(f"📈 Batch size: {args.batch}, LR head: {args.lr-head}, LR full: {args.lr-full}")
    ...
    optimizer = torch.optim.AdamW(
        ...
        lr=args.lr-head, weight_decay=1e-4,
    )
    scheduler = None
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch == args.warmup_epochs + 1:
            ...
            optimizer = torch.optim.AdamW(
                model.parameters(), lr=args.lr-full, weight_decay=1e-4,
            )
            remaining = args.epochs - args.warmup_epochs
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                optimizer, T_max=remaining, eta_min=1e-6,
            )
            print(f"📈 Using cosine LR from {args.lr-full} to 0 over {remaining} epochs")
