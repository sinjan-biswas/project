...
class DrivingLicenseParser(DocumentParser):
    ...
    def parse(self, lines, full_text):
        f = {"name": None, "dl_number": None, "date_of_birth": None,
             "validity_from": None, "validity_to": None, "vehicle_class": None}
        text = "\n".join(l["text"] for l in lines)

        m = re.search(r"\b([A-Z]{2}[-\s]?\d{2}[-\s]?\d{4}[-\s]?\d{7})\b", text.upper())
        if m: f["dl_number"] = re.sub(r"\s", "", m.group(1))
        ...
        if m: f["vehicle_class"] = m.group(1).strip()
        alpha = [l["text"].strip() for l in lines
                 if re.match(r"^[A-Za-z][A-Za-z .]{2,40}$", l["text"].strip())]
        if alpha: f["name"] = alpha[0].title()

        return {"document_type": "driving_license", "fields": f, "confidence": 0.7}